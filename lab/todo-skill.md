...
**Наш формат фаз (проверен на weekly-planning, 2026-03-23):**

Каждая фаза скилла — по шаблону:
...
Что показать пользователю: {перечень артефактов}.
Output должен требовать данные из каждого шага алгоритма —
тогда пропуск шага = неполный output = видно.

**После «ОК» — {что записать в файл}.**
```

**Дополнения из практики:**
- **Iron Law** — одно правило-якорь в начале скилла. Для многофазных: «каждая фаза = отдельное сообщение, жди ОК»
- **Итерации** — для фаз с творческим output (план, пост): не одно ОК, а цикл драфт → правки → обновлённый → финал. Указывать явно
- **Субагент для сбора данных** — если фаза требует тяжёлого сбора из нескольких источников, вынести в субагента (context: fork). Не вызывать другой скилл в том же контексте — два набора инструкций конкурируют
- **Edge Cases / Red Flags** — при создании скилла оставлять шаблоны с TODO. Заполнять только из реальных сбоев после тестирования. Выдуманные edge cases бесполезны
- **«Принцип» вместо «роли»** — «ты стратегический партнёр» ничего не значит для агента. Вместо этого: одна строка что конкретно важно при работе (пример: «Сформировать план недели, который ведёт к цели декады»)
