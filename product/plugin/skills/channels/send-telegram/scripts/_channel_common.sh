...
# намеренно нет: один путь, а не «ещё один способ попасть туда же».
...
channel_report_route() {
  ...
  # вторая — значение, потому что переменная задана: в терминал уезжает сам
  # токен бота.
  ...
  echo "  а список адресатов отдаёт инструмент list_contacts."
}
