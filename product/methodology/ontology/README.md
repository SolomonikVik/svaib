# Ontology — модель управления и её сущности

Модель того, как svaib видит управление компанией, и её детализация до отдельных управленческих сущностей. Протоколы работы → `../`; готовые шаблоны → `../../plugin/skills/scaffold/template/`.

## Source of truth

- [Svaib Management System](01_svaib-management-system.md) — **SOT** базовой модели управления, на которой строится логика продукта svaib.

## Вторичные файлы

- [entities.md](entities.md) — рабочий каталог управленческих сущностей; будет пересобран после принятия базовой модели.
- [behavioral_patterns.md](behavioral_patterns.md) — специализированный каталог поведенческих паттернов на встречах.
- [protocol_format.md](protocol_format.md) — схема семейства протоколов: маппинг сущностей на секции и общие правила.

## Устаревшие файлы

Эти файлы не являются SOT и ожидают синхронизации или архивации:

- [01_ontology-draft.md](01_ontology-draft.md) — первый черновик интегральной онтологии.
- [ontology.md](ontology.md) — прежняя полная спецификация.
- [management_cycles.md](management_cycles.md) — прежняя модель девяти управленческих циклов.
- [rituals.md](rituals.md) — прежний каталог ритуалов.

Оставшаяся работа — в [_plan.md](_plan.md). Канон формата файла — [02_file-spec.md](../scaffold/02_file-spec.md).
