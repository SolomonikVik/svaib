...
> **⚠️ АРХИВ (10.02.2026):** n8n Cloud отключён. Workflows сохранены в `dev/n8n_backup/` (JSON-файлы). Этот файл описывает реализацию, которая работала в декабре 2025 — январе 2026. При восстановлении (self-hosted n8n) — использовать как референс.

## Кратко

Реализация системы svaib: потоки данных между компонентами, структура n8n workflows, контракты AI-промптов, маппинг плейсхолдеров к источникам данных. Этот файл отвечает на вопрос "КАК работала система" (n8n Cloud отключён 10.02.2026, backup в `dev/n8n_backup/`).
