...
## Сохранность содержания при трансформации текста

Reference-based eval суммаризации или подробного очищения текста — отдельный класс, который нельзя сводить к свободному баллу 0–100.

- Gold-set размечается как атомарные единицы содержания: SCU в Pyramid, ACU в автоматизируемых вариантах.
- `source → output` измеряет coverage/recall и ловит пропуски.
- `output → source` измеряет faithfulness/content precision и ловит добавления и искажения.
- Match сопровождается evidence spans; для диалогов в смысл единицы входят speaker attribution и modality.
- Сначала применяются task-specific hard gates, затем показывается dashboard отдельных осей; среднее не должно скрывать критический дефект.
- QA-based QAEval/QuestEval полезны как вспомогательный сигнал, а LLM matcher калибруется на человеческом gold.

Схема разметки, ограничения автоматизации и граница с остальным meeting pipeline — в [summarization-evaluation.md](summarization-evaluation.md).
