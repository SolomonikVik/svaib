...
tags: [skills, index, marketplace, ecosystem, skill-graph, patterns, tooling, code-enforcement, rule-enforcement, hooks, fallback, verification, claude-5]
publish: false
---
...
Skills — структурированные инструкции в формате SKILL.md, которые AI загружает по необходимости и следует им. Не код — "учебники" для AI. Делают AI специалистом в конкретной области без повторения инструкций каждый раз. Открытый стандарт Anthropic (декабрь 2025), принятый OpenAI, Vercel и 40+ AI-агентами. Экосистема: официальный репо Anthropic, CLI `npx skills`, агрегаторы. Автоматическая активация ненадёжна (~20%), детали: [skill-activation.md](skill-activation.md). Правило, написанное в скилле, не эквивалентно правилу, которое работает: класс enforcement и его проверка — [rule-enforcement.md](rule-enforcement.md). Знания о Skills — рабочий материал для продукта SVAIB (см. product/01_overview.md).
...
**Правило обязано назвать свой класс enforcement.** Инструкция «ОБЯЗАТЕЛЬНО делай X» сама по себе ничего не удерживает: правило либо стоит на механизме, который ловит нарушение (**регламент**), либо осознанно держится на суждении модели (**культура**), либо привязано к каденции и речевому триггеру (**процесс**). Декларация сильнее механизма — дефект: читатель строит на правиле как на инварианте, которого нет. Обратная ошибка — хук там, где по природе работает суждение. Правило не считается завершённым, пока не названы три критерия: точка enforcement, каденция, сигнал деградации. Воронка, подкласс «будильник» и ловушки проектирования проверок — [rule-enforcement.md](rule-enforcement.md).
