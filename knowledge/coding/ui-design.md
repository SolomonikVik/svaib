...
### Библиотеки референсов — для людей
...
### Библиотеки референсов — для агентов через MCP (новая категория)

**Сдвиг паттерна:** дизайн-каталоги получают MCP-интерфейсы. Не "человек ищет скрин и закидывает в промпт", а "агент сам ищет референсы под задачу через MCP". Заземляет AI-агента на реальные продуктовые паттерны вместо generic-дизайна (см. раздел "AI-слепота" ниже).

| Инструмент | Суть | Формат для агента | Цена | Сайт |
|-----------|------|-------------------|------|------|
| **Lazyweb** | 257-300K+ скринов мобильных и веб-приложений, user flows. Автор Ali Abouelatta. Позиционируется как "design taste для AI" | MCP-сервер + бесплатный токен. Без rate-limit для агентов. Документация под Claude: lazyweb.com/claude.md | Бесплатно | lazyweb.com |
| **Refero (Refero MCP + Refero Styles)** | Поиск по бренду/настроению/цвету/типографике/URL. Стили от Apple, Linear, Stripe, Anthropic, ElevenLabs и др. | **Refero MCP** — для Claude/Cursor/Windsurf. Стиль выгружается как **DESIGN.md** (цвета, типографика, spacing, компоненты) — агент читает как обычный файл | Beta | styles.refero.design |

**Когда что использовать:**
- Mobbin/Dribbble — когда дизайнером работает человек, ищешь насмотренность глазами
- Lazyweb/Refero MCP — когда агент пишет код, нужно дать ему "вкус". Подключаешь MCP, агент сам поднимает релевантные референсы при генерации UI
...
- **MCP-референсы для агента** (Lazyweb, Refero MCP) — агент сам поднимает релевантные паттерны при генерации, не работает "из головы". См. раздел "Библиотеки референсов — для агентов через MCP"
...
- [../tools/html-output.md](../tools/html-output.md) — соседняя ось: вывод **данных** как HTML-артефакт (отчёт/дашборд/дек) ≠ генерация **интерфейса продукта** здесь. Антипаттерн «AI-слепоты» (generic-дизайн) там описан как «distributional convergence» с anti-slop приёмами
