...
### Контекст-инъекции в VSCode extension (эксперимент svaib, март 2026)

Экспериментально измерено, что именно инжектируется в контекст Claude Code при работе через VSCode extension и сколько стоит.

**Фиксированная стоимость сессии (~15% из 200k):**

| Что | Токены | Поведение |
|-----|--------|-----------|
| System prompt | ~5.9k | Статичен, один раз |
| System tools | ~7.6k | Статичны |
| Deferred tools (схемы незагруженных инструментов) | ~9.8k | Статичны |
| CLAUDE.md (корневой) | ~3.1k | Статичен, НЕ множится на каждый ход |
| MEMORY.md | ~9 токенов (индекс) | Статичен |
| Skills (описания триггеров) | ~2.8k (34 скилла) | Статичны |
| Agents (описания) | ~200 | Статичны |
| Autocompact buffer | ~33k | Зарезервировано системой |

**Динамическая стоимость за ход:**

| Инъекция | Токены за ход | Триггер | Повторяется |
|----------|---------------|---------|-------------|
| Чистая переписка (вопрос + ответ) | ~1.0-1.2k | Всегда | Да |
| **File modification reminder** | Пропорционально diff (~0.5-3k) | После Read/Edit файла Claude'ом | Да, на КАЖДЫЙ ход |
| **Подпапочный CLAUDE.md → @README** | ~1.5k за папку | Read любого файла в папке | Да, на КАЖДЫЙ Read |
| /context таблица | ~0.9k | Вызов /context | Каждый вызов |
| @ файл в чате | = размер файла | Пользователь прикрепил | Контролируемо |
| ide_selection (выделенный текст) | = размер выделения | Пользователь выделил | Контролируемо |
| ide_opened_file (уведомление) | ~20 | Пользователь открыл таб | Только уведомление |

**File modification reminders — главный скрытый пожиратель:**
- Триггер: Claude прочитал (Read) или отредактировал (Edit) файл. После этого file watcher VSCode extension на КАЖДЫЙ последующий ход инжектирует `<system-reminder>` с diff'ом этого файла
- Размер пропорционален области изменения (контекст вокруг каждого изменённого места), НЕ всему файлу
- Файлы изменённые пользователем вне Claude НЕ отслеживаются, если Claude их не читал в этой сессии
- Каскадно подгруженные файлы (через @README в подпапочных CLAUDE.md) тоже НЕ отслеживаются
- Накопление: 5 файлов × 1k diff = +5k токенов на КАЖДЫЙ ход = 2.5% контекста за ход
- Известные issues: [#4464](https://github.com/anthropics/claude-code/issues/4464), [#9388](https://github.com/anthropics/claude-code/issues/9388), [#21693](https://github.com/anthropics/claude-code/issues/21693), [#9614](https://github.com/anthropics/claude-code/issues/9614)

**Каскадная загрузка через подпапочные CLAUDE.md:**
- Если в папке есть CLAUDE.md с директивой `@README.md`, то при КАЖДОМ Read любого файла из этой папки платформа инжектирует полный README.md (~1-1.5k токенов)
- Не кэшируется, не дедуплицируется — каждый Read = повторная загрузка

**Что НЕ влияет на контекст:**
- Открытые табы в VSCode (только уведомление ide_opened_file ~20 токенов)
- Файлы modified в git, но не прочитанные Claude в текущей сессии
- Файлы изменённые пользователем вне Claude (если Claude их не читал)

**Контекст-гигиена перед длинной сессией:**
1. Минимизировать количество файлов, которые Claude читает/редактирует
2. Удалить подпапочные CLAUDE.md с `@README.md` (бесплатная экономия)
3. Для чистых ревью-сессий без правок — рассмотреть CLI вместо VSCode extension (нет file watcher)
4. Мониторить через `/context` — показывает текущее потребление по категориям

Источник: эксперимент svaib, 12 марта 2026. Серия из 25+ замеров с контролируемыми переменными.

### Подгрузка файлов в промпт: VS Code extension vs CLI

| Синтаксис | VS Code extension | CLI |
|-----------|-------------------|-----|
| `@file.md` в тексте промпта | Резолвит путь, инжектирует содержимое файла в контекст | Остаётся текстом, не подгружает |
| `` `!cat file.md` `` | Preprocessing: выполняет команду, вставляет результат | То же — preprocessing работает везде |

**Практика:** если работаешь в CLI и нужно подгрузить файл в промпт — используй `` `!cat path/to/file.md` ``. `@file` — только для VS Code extension.

Источник: эксперимент svaib, 18 марта 2026.
...
- **Перед заведением хука** — прогнать правило по накопленным транскриптам всухую: сколько раз оно сработало бы и сколько из них ложно. Класс правила, три критерия его завершённости и типовые ловушки проектирования проверок — [../skills/rule-enforcement.md](../skills/rule-enforcement.md)
...
Применения из практики Anthropic: миграции кода (Bun переписан с Zig на Rust через workflows — один промпт, 11 дней, детали кейса → [../prompting/model-elicitation.md](../prompting/model-elicitation.md)), ранжирование больших наборов (1000+ элементов турниром), root cause analysis независимыми гипотезами, триаж очередей, проверка документов на соответствие CLAUDE.md специализированными верификаторами.
...
**Потолок параллельности (замер 28.08.2026).** Одновременных `agent()` в одном workflow — `min(16, число CPU − 2)`: на 4-ядерной машине это 2, остальные вызовы ждут в очереди и не теряются; документированного override нет. У обычного инструмента `Agent` свой потолок — `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` в `env` settings.json (по умолчанию 20), от CPU он не зависит. Практика: параллельных исполнителей запускать несколькими `Agent` одним сообщением, Workflow оставлять конвейерам «найти → проверить»; на малом числе ядер узкое место — не агенты, а локальные тесты с docker в параллельных worktree.
...
Повторяющаяся задача по расписанию (cron для Claude): **loop** исполняется локально, **routine** — в облаке (машину можно выключить). Отличие от dynamic workflow: workflow — одна задача, разбитая на стадии; loop/routine — одна и та же задача, запускаемая снова и снова (раз в 5 минут, час, день) без переноса контекста между запусками. Практика Anthropic: ~20–30 ежедневных routines сами обслуживают кодовые базы — чистка dead code, авто-ship раскатанных экспериментов, дописывание/удаление тестов, унификация дублирующихся абстракций (→ [../prompting/model-elicitation.md](../prompting/model-elicitation.md)).
...
1. **Насыщенный параллелизм деградирует исполнение.** При множестве одновременных `claude -p`-подпроцессов часть (кластерно, обычно в конце волны запуска) возвращает `rc=0`, но модель фактически не читает промпт — пустой diff/no-op + короткий вывод или обрывок системного текста вместо реальной работы. Не баг конкретного скрипта-обёртки — общее свойство headless-режима под нагрузкой. Митигация: детектировать сигнатуру (пустой результат + аномально короткая длительность/вывод) и ретраить, держать внешний параллелизм запуска в разумных пределах (не десятки одновременных `claude -p`).
2. **Делегирование субагенту (`Agent`/`Task`) работает, включая параллельный fan-out (обновлено 28.08.2026, Эрик).** История: вывод 17.07 «структурно не работает, handoff classifier» не подтвердился; перепроверка 22.07 (Claude Code 2.1.217) подтвердила одиночное foreground-делегирование (реальный прогон: 1146-строчный транскрипт, opus, 594с, все файловые правки на месте) и зафиксировала «параллельный fan-out нескольких `Task` в non-TTY headless виснет» ([issue #56540](https://github.com/anthropics/claude-code/issues/56540)). **Это наблюдение устарело:** на текущем Claude Code и в Claude Agent SDK (programmatic `agents={...}`) параллельные субагенты работают — правило «один субагент за раз» снято, в том числе в `prompt.md` autonomous-runner. Действующие границы: (а) `Agent` умеет async/background-запуск, но **фоновый субагент может не пережить завершение головной `-p`-сессии** — только foreground с ожиданием результата; (б) прогон с делегированием заметно дольше инлайн-варианта — закладывать таймауты с запасом. Протокол перепроверки 22.07 — `dev/evals/meeting-analysis/_private/runs/subagent-recheck-2026-07-22/`.
3. **Промпт может случайно самоактивировать существующий Skill.** Если текст промпта достаточно похож на описание/триггер установленного Skill, Claude Code инжектит `<skill-instructions>` поверх пользовательского промпта — модель уходит читать инструкции скилла вместо содержательной работы. Фикс: `--disable-slash-commands` в headless-вызове, если скиллы для задачи не нужны (напр. пайплайн читается напрямую через `Read`, а не через активацию Skill).
