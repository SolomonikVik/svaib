...
2. **Делегирование субагенту (`Agent`/`Task`) структурно не работает без живого человека.** Есть отдельный «handoff classifier at delegation» в Claude Code auto mode, независимый от permission-mode — `--dangerously-skip-permissions` его НЕ снимает для реального делегирования внутри содержательной сессии (при этом подтверждённо работает для изолированного тривиального спавна субагента в пустой директории/отдельном worktree — это разные сценарии). В headless-режиме нет UI, чтобы спросить человека при handoff — вызов зависает/обрывается. Вывод: план работы для `claude -p` не должен рассчитывать на делегирование субагенту — прикладная логика применяется инлайн, в одной сессии.
