...
tags: [claude-code, tools, plugins, mcp, hooks, anthropic]
publish: false
version: 2
---
...
Hooks позволяют запускать скрипты на события жизненного цикла Claude Code:
- **SessionStart / SessionEnd** — начало/конец сессии
- **PreToolUse / PostToolUse** — до/после вызова инструмента
- Другие события (agent hooks, prompt hooks)

Пример: Superpowers использует `session-start.sh` для инициализации при каждом запуске.

Конфигурация в `hooks/hooks.json`:
```json
{
  "hooks": [
    {
