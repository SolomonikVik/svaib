...
2. **Делегирование субагенту (`Agent`/`Task`) работает — но только по одному и foreground (перепроверено 22.07.2026, Claude Code 2.1.217; исходный вывод 17.07 «структурно не работает, handoff classifier» устарел и внешне не подтвердился).** Одиночное foreground-делегирование в `claude -p` работает надёжно — подтверждено и тривиальным спавном, и реальным содержательным прогоном (1146-строчный транскрипт, opus, 594с, все файловые правки на месте). Действующие границы: (а) **параллельный fan-out нескольких `Task` в одном сообщении в non-TTY headless виснет** — известный баг, Anthropic чинить не планирует ([issue #56540](https://github.com/anthropics/claude-code/issues/56540), closed not planned); правило — один субагент за раз; (б) `Agent` теперь умеет async/background-запуск — **фоновый субагент может не пережить завершение головной `-p`-сессии**, поэтому только foreground с ожиданием результата; (в) прогон с делегированием заметно дольше инлайн-варианта — закладывать таймауты с запасом. Если нужен настоящий параллельный fan-out субагентов headless — путь не `claude -p`, а Claude Agent SDK (programmatic `agents={...}`, субагенты работают нативно). Протокол перепроверки — `dev/evals/meeting-analysis/_private/runs/subagent-recheck-2026-07-22/`.
