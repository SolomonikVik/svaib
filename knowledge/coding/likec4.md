...
- **`variant sequence` в dynamic view** — рендер как UML-sequence с пошаговым проигрыванием (next/back). Блоки `parallel` · `opt` · `loop` · `break` · `alt/when/else` · `try/catch/finally` рисуются вложенными рамками и работают как группировка шагов: `opt 'условие'` честно показывает необязательный этап процесса. Отдельного ключевого слова `group` нет — группируют именно эти блоки.
- **`navigateTo <view>` на шаге** разворачивает его в отдельную диаграмму. Так строится верхний уровень из крупных этапов, каждый из которых раскрывается по клику в свой dynamic view.
...
- **`description` элемента должен идти до вложенных элементов.** Свойство после закрывающей скобки вложенного элемента роняет парсер с невнятным «Expecting token of type '}' but found `description`».
- **Связь родителя с собственным ребёнком запрещена** — `parent -> parent.child` даёт «Invalid parent-child relationship». В dynamic view переход внутрь контура выражается шагом от ребёнка наружу или `navigateTo`.
- **Вложенные `parallel` не допускаются** — уплощаются в один блок; остальные блоки потока вкладываются свободно.
