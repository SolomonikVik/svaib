...
Конкретный пример этого паттерна на самом Claude Code — headless `claude -p` (CI/харнесы, не интерактивная сессия): свой набор ограничений (деградация под параллелизмом, делегирование субагенту не работает без человека, риск случайной самоактивации Skill) → [claude-code.md → Headless-режим](claude-code.md#headless-режим-claude--p).
...
- [../agents/workflow-automation.md](../agents/workflow-automation.md) — тот же принцип «агент на build-time, не в рантайме», применённый к конфигам автоматизации (агент пишет workflow)
