...
**LightRAG** — облегчённая open-source реализация той же идеи (HKUDS, EMNLP 2025): entity-граф через LLM-экстракцию, без иерархических community summaries. Первое собственное эмпирическое исследование (экономика, дефекты, позиционирование) → [lightrag.md](lightrag.md).
...
В контекстной архитектуре Second Value AI Brain (→ workbench.md) RAG занимает позицию **неструктурированного поиска** — когда агент не знает маршрут и ищет по смыслу. Два режима:
...
Приоритетная реализация для тестирования: QMD (→ lab/_inbox/qmd-research.md).
...
- [lightrag.md](lightrag.md) — LightRAG: облегчённый GraphRAG, собственный эмпирический разбор (экономика, дефекты)
