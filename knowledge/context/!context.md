...
Знания о context engineering, RAG и memory — рабочий материал для продукта SVAIB (см. product/01_overview.md).
...
Паттерн (Karpathy, 2026): вместо RAG по сырым источникам — LLM инкрементально компилирует markdown-вики. Три операции: **Ingest** (обработать источник → обновить 10-15 страниц), **Query** (ответ на вопрос становится страницей вики — знание компаундится), **Lint** (периодический аудит целостности: противоречия, устаревшие факты, осиротевшие страницы). Ключевой инсайт: LLM отлично справляется с бухгалтерией знаний (кросс-ссылки, консистентность), от которой человек отказывается из-за экспоненциального роста maintenance burden. Паттерн работает на уровнях 0-1 спектра (markdown + git, без БД). Вызвал волну реализаций: llm-wiki-kit (MCP), sage-wiki, memex, atomic-knowledge, Basic Memory (OSS + облако, MCP, индекс full-text/семантика/граф). Подробнее → [llm-wiki.md](llm-wiki.md)

### Context Graphs — институциональная память решений

Концепция Foundation Capital (Ashu Garg, январь 2026): context graph = живая карта того, КАК организация принимает решения. "Decision traces" — записи reasoning за решениями (какие исключения, какой прецедент, кто одобрил). Модели commoditize, а decision traces — компаундящий proprietary актив. Агенты создают traces автоматически: траектория агента по системам = decision trace. Со временем из траекторий возникает мировая модель организации. Открытые вопросы: темпоральность (→ решается через [temporal-graphs.md](temporal-graphs.md)), governance, где хранить граф. Для SVAIB: наш product/ontology — по сути context graph для CEO. Подробнее → [context-graphs.md](context-graphs.md).

### Права доступа как часть контекстной архитектуры

Отдельный слой, который в file-first системах обычно вспоминают последним: что из найденного пользователю вообще можно показать. Три модели доставки контента — предындексация (быстро и качественно, но создаёт вторую копию данных и вторую, отстающую копию прав), live-запрос в источник (права всегда актуальны, возможностей меньше), выдача по метаданным ссылки под конкретного пользователя. Ключевое правило: **индекс отвечает за скорость, не за безопасность** — права проверяются в момент запроса; документированный контрпример — Nextcloud Context Chat, отвечающий по индексу в обход File Access Control. Для агента чтение уже равно выносу смысла, поэтому права на чтение и на действия разводятся отдельно. Проверять зрелость надо на изменениях entitlements (новый сотрудник, отзыв временного доступа, ушедший подрядчик), а не на demo. Механика → [permission-aware-retrieval.md](permission-aware-retrieval.md); агент как субъект прав → [../agents/agent-authorization.md](../agents/agent-authorization.md); что предлагают готовые платформы → [../tools/team-content-platforms.md](../tools/team-content-platforms.md).

### Архитектура markdown-native систем знаний — сборка всех слоёв

Синтез поверх остальных тем папки: как из открытых markdown-файлов собрать систему, с которой работают и люди, и агенты. Класс систем не имеет единого имени — file-over-app (файлы переживают приложения), local-first (владение и место исполнения), markdown-first knowledge base (формат и нативное чтение агентом), LLM wiki (операционный цикл), OKF (vendor-neutral упаковка знаний организации от Google Cloud); ICM — **не** название класса, это методология оркестрации этапов работы агента через папки. Главный тезис: такие системы ломаются не на качестве модели, а на слипании плоскостей, когда файловое дерево назначают одновременно хранилищем, поисковым индексом и моделью прав. Развести на четыре: содержимое (хранение, версии, экспорт) · контекст (поиск, связи, подача в окно) · права (кто и что вправе, включая агента) · действия (запись, публикация, guardrails, трасса). → [md-data-systems.md](md-data-systems.md)
...
- [permission-aware-retrieval.md](permission-aware-retrieval.md) — как права доступа доезжают до выдачи AI: три модели доставки контента, задержка ACL→индекс, чтение ≠ действие, edge cases
- [md-data-systems.md](md-data-systems.md) — архитектурный синтез: четыре плоскости markdown-native системы знаний, терминология класса, развилки проектирования
