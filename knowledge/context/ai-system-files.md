...
## Импорты и автозагрузка: Claude Code vs Codex
...
**Ловушка.** Codex **не грузит `AGENTS.md` ниже cwd** (вложенные — issues codex#12115, codex#13288). В IDE чат стартует от корня воркспейса → папочный `AGENTS.md` глубоко открытого файла в контекст не попадёт. Открытая вкладка ≠ загруженный AGENTS. Аудит загрузки: `codex -c log_dir=./.codex-log` → `codex-tui.log`.

**Как заставить агента прочитать README/инструкцию папки.** Текстовая инструкция — мягко (агент может проигнорировать). Механически: Claude — `@README.md` в `CLAUDE.md`; Codex — самодостаточный `AGENTS.md` либо явное правило чтения. Жёстко в обоих — **хук**: Claude `SessionStart`/`UserPromptSubmit` инжектят `additionalContext` в контекст (JSON `hookSpecificOutput.additionalContext` или stdout); Codex-сторона требует проверки.

За актуальной механикой — офиц. доки: [Claude Code — Memory/imports](https://code.claude.com/docs/en/memory), [Claude Code — Hooks](https://code.claude.com/docs/en/hooks), [Codex — AGENTS.md](https://developers.openai.com/codex/guides/agents-md).
...
- [Claude Code — Memory & imports](https://code.claude.com/docs/en/memory) — `@`-импорты, иерархия CLAUDE.md (актуальная механика)
- [Claude Code — Hooks](https://code.claude.com/docs/en/hooks) — SessionStart/UserPromptSubmit, additionalContext
- [Codex — AGENTS.md](https://developers.openai.com/codex/guides/agents-md) — загрузка, merge root→cwd, override, лимиты (актуальная механика)
- [codex#17401](https://github.com/openai/codex/issues/17401) — запрос `@include` для AGENTS.md (статус: не реализован)
