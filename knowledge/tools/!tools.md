...
Платформы автоматизации и AI-инструменты, НЕ связанные с написанием кода. Не путать с: coding/ (IDE, кодинг-ассистенты) и agents/ (протоколы, SDK).
...
Среды, через которые пользователь работает с AI. Для SVAIB это интерфейс клиента: мы наполняем оболочку содержимым. Каталог: Claude Code, Cowork, OpenCode, Codex, Kojori, VS Code. → [ai-workspaces.md](ai-workspaces.md)
