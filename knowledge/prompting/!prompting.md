...
## Промптинг под поколение Claude 5

Смена поколения моделей переписывает часть правил ниже. Главное: следование инструкциям выросло настолько, что поведение правится одной короткой фразой про намерение вместо перечня частных запретов — а старые перечни активно вредят. Основной рычаг качества сместился в отдельный параметр **effort** (интеллект ↔ латентность ↔ цена), низкие уровни нового поколения часто превосходят максимальный уровень прежнего. Длинные ходы стали нормой: один запрос идёт минутами, автономный прогон — часами, что ломает синхронные харнесы. Появился отдельный класс типовых сбоев со своими лечениями (раннее завершение хода без вызова инструмента, тревога о бюджете контекста, фабрикация прогресса) и жёсткий антипаттерн — просить модель воспроизвести собственные рассуждения в тексте ответа: это попадает под категорию отказов и роняет запрос на фолбэк.
...
- **context/** — порядок контента в промпте, per-model tuning = пересечение с context engineering; смена правил при смене поколения → [../context/context-engineering-claude5.md](../context/context-engineering-claude5.md)
