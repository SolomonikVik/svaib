...
5. **Сверь grep с категориями.** Если grep нашёл файлы в категории, которую ты не рассматривал — значит ты пропустил категорию. Перечисли ВСЕ затронутые категории (из README + из grep) прежде чем формировать ориентировку.
...
**Принципы записи:**
- **Пропорциональность.** Размер правки пропорционален размеру новой информации. Одна новая фича = одна строчка в целевом файле. Три теста: (1) меняет суть инструмента? Нет — не трогай "Кратко". (2) Отдельная тема? Нет — не создавай секцию, допиши строку. (3) Меняет общую картину категории? Нет — не трогай !файл.
- **Без быстро устаревающего.** Не записывай: точные числа stars, версии моделей (GPT-5.3, Claude 4.5), количества скиллов/плагинов, позиции в рейтингах, цены, хронологии релизов, таблицы GitHub issues, roadmap-статусы ("в разработке", "анонсировано"). Записывай стабильное: что это, чем отличается, как использовать, кто автор. Качественная оценка ("крупный проект") вместо количественной ("58k stars")
- **Дельта, не копия.** Если у инструмента есть официальная документация — ссылайся на неё, не дублируй. Записывай то, чего в доке нет: community-паттерны, практика применения в нашем проекте, принципы выбора ("когда что использовать"). Наша копия устареет, документация — нет. Следствие: есть ссылка на доку → суть в 1-2 предложения + ссылка, не пересказ. Это же правило внутри knowledge/: если тема подробно раскрыта в файле X — не дублируй в файле Y, ставь ссылку
