...
- **GUI-задачи → используй `claude --chrome -p`:** Любая настройка в браузере (n8n Dashboard, Google Cloud Console, Supabase Dashboard, Telegram Web) — вызывай через `claude --chrome -p "задача"`. НЕ пиши пошаговые инструкции Виктору. Виктор только вставляет секреты из KeePass.
