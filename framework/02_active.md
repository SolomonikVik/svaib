...
**Ближайшие шаги.**
- [ ] Шаг 1.5: переписать существующий черновик брифа CEO Клиента 1 (`clients/private/lebedev/docs/2026-05-12_canonical_metrics_brief.md` — помечен ⚠️ УСТАРЕЛО, был под старый формат: 3 поля метрики, фиксированная колонка «между B и C», ссылка на удалённый `canonical_metrics_ssp.md`) под новый канон L1 (5 полей, имя служебной колонки выбирает клиент, артефакт `business-metrics.md`). Параллельно — промпт-помощник работы с метриками (скилл для формирования `business-metrics.md` у нового клиента).
