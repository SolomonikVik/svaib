...
Methodology metrics переосмыслена через рамку слоёв. **Первый слой** (минимум у любого клиента) — каноническое имя метрики, колонка с этим именем в xlsx, помощник, который по этой колонке достаёт число. Без паспортов, без `direction`, без extractor-скрипта, без narrative. Появился новый файл [methodology/metrics/first-layer.md](methodology/metrics/first-layer.md) с рамкой, DoD и форматом `canonical_metrics.md`. [rollout.md](methodology/metrics/rollout.md) переписан с 12-шагового pilot на 6-шаговый playbook первого слоя. [HOWTO.md](methodology/metrics/HOWTO.md) переписан под сценарий «вопрос CEO → каноническое имя → число». [README.md](methodology/metrics/README.md) — карта без статусности, точка входа `first-layer.md`. [intake-form.md](methodology/metrics/intake-form.md) понижен до внутреннего чек-листа координатора. В [scaffold/05_metrics/](scaffold/05_metrics/) добавлен шаблон [canonical_metrics.md](scaffold/05_metrics/canonical_metrics.md); README папки помечает его как стартовую точку. В [methodology/metrics/architecture.md](methodology/metrics/architecture.md) поставлен маяк на `first-layer.md` в «Кратко»; основное тело архитектуры (паспорта, маршруты, snapshot, eval) сохранено как база для второго и третьего слоёв.
